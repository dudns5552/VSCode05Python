...
'''문자에서 여러줄에 걸쳐 문자열을 쓸때는 이와 같이
더블이나 싱글 쿼테이션을 사용하면된다. 할당할 변수가 없으면 주석처럼
사용된다.'''
...
print(engStr[:3]) #시작인덱스가 없으면 처음부터 시작. 0~2 : Hel
print(engStr[1:3]) #1~2 : el
print(engStr[1:]) #종료인덱스가 없으면 끝까지 1~end
